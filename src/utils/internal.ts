...
export const createTemplate = <const K extends readonly PropertyKey[]>(
	keys: K,
	isValue: (key: K[number]) => AnyTypeGuard,
) => {
...
export const pick = <T, K extends readonly (keyof T)[]>(
	...
	keys: K,
): Pick<TypeGuardTemplate<T>, K[number]> => {
	return createTemplate(keys, key => Reflect.get(template, key)) as Pick<
		TypeGuardTemplate<T>,
		K[number]
	>;
};

export const omit = <T, K extends readonly (keyof T)[]>(
	template: TypeGuardTemplate<T>,
	keys: K,
): Omit<TypeGuardTemplate<T>, K[number]> => {
